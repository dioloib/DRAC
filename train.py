...
from segmentation_models_pytorch import Unet, UnetPlusPlus, Linknet, FPN, PAN, DeepLabV3
import random
...
def get_segmentation_model(model_name, encoder_name, encoder_weights, classes, in_channels, activation=None):
    """
    根据提供的模型名称返回一个分割模型实例。
    """
    if model_name == "Unet":
        ...
        raise ValueError("Invalid model name. Supported models are Unet, Unet++, FPN, PAN, DeepLabV3.")
...
if __name__ == '__main__':
    ...
    #     activation=ACTIVATION,
    # )
    model = get_segmentation_model('FPN', ENCODER, ENCODER_WEIGHTS, len(CLASSES), 3, ACTIVATION)
